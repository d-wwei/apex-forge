import { mkdirSync, rmSync, existsSync, copyFileSync, writeFileSync, statSync } from "fs";
...
export async function createWorkspace(taskId: string, root: string = ".workspaces"): Promise<WorkspaceInfo> {
  ...
  if (isGitRepo(process.cwd())) {
    ...
    const result = spawnSync("git", ["worktree", "add", wsPath, "-b", branchName], {
      encoding: "utf-8",
      timeout: 10000,
    });
    ...
    console.warn(`[workspace] git worktree add failed for ${taskId}, falling back to plain directory: ${(result.stderr || "").trim()}`);
...
export async function cleanupWorkspace(workspacePath: string): Promise<void> {
  ...
  const isWorktree = existsSync(dotGitPath) && !statSync(dotGitPath).isDirectory();
  if (isWorktree) {
    const result = spawnSync("git", ["worktree", "remove", workspacePath, "--force"], {
      encoding: "utf-8",
      timeout: 10000,
    });
