...
function setupApex(stage: string, extras?: Record<string, unknown>): void {
  ...
  writeFileSync(
    join(testDir, ".apex", "state.test-session.json"),
...
beforeEach(() => {
  ...
  testDir = join(
    tmpdir(),
    `apex-gate-test-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  );
...
describe("Git operations gate", () => {
  ...
  test("git commit in ship stage → allow", () => {
    setupApex("ship", {
      ship_checkpoints: ["iteration-summary", "push-prompt"],
    });
    const r = runHook({
      tool_name: "Bash",
      tool_input: { command: "git commit -m feat" },
...
describe("Brainstorm code edit gate", () => {
  ...
  test("Edit .md file in brainstorm → allow", () => {
    ...
    const r = runHook({
      tool_name: "Edit",
      tool_input: { file_path: "docs/brainstorms/req.md" },
      cwd: testDir,
    });
    expect(parseDecision(r.stdout)).toBeNull();
  });
  ...
  test("Edit docs/ path in brainstorm → allow", () => {
    ...
    const r = runHook({
      tool_name: "Write",
      tool_input: { file_path: "docs/plans/plan.md" },
      cwd: testDir,
    });
    expect(parseDecision(r.stdout)).toBeNull();
  });
