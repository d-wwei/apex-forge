/**
 * apex-forge CLI — thin wrapper that talks to the persistent server
 *
 * Flow:
 *   1. Read .apex/browse.json for port + token
...
// Determine if we're running as a compiled binary
...
export function resolveServerScript(
  ...
  // Compiled binary: derive the source tree
  if (execPath) {
    const adjacent = path.resolve(path.dirname(execPath), '..', 'src', 'browse', 'server.ts');
    if (fs.existsSync(adjacent)) return adjacent;
    const adjacent2 = path.resolve(path.dirname(execPath), '..', 'src', 'server.ts');
    if (fs.existsSync(adjacent2)) return adjacent2;
  }

  // Compiled mode: no external script needed — use __server__ self-invoke
...
const SERVER_SCRIPT = resolveServerScript(); // null in compiled mode
...
function cleanupLegacyState(): void {
  // No legacy state on Windows — /tmp and `ps` don't exist, and apex-forge
...
async function startServer(extraEnv?: Record<string, string>): Promise<ServerState> {
  ...
  } else if (IS_COMPILED || !SERVER_SCRIPT) {
    // Compiled binary: self-invoke with __server__ flag
    proc = Bun.spawn([process.execPath, '__server__'], {
      stdio: ['ignore', 'pipe', 'pipe'],
      env: { ...process.env, BROWSE_STATE_FILE: config.stateFile, ...extraEnv },
    });
    proc.unref();
  } else {
    // Dev mode: bun run server.ts
...
async function main() {
  ...
  // Compiled binary self-invoke: run as daemon server
  if (args[0] === '__server__') {
    // Importing server.ts triggers start() which starts the HTTP server + Playwright
    await import('./server');
    return; // Server keeps running via Bun.serve event loop
  }

  if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
    console.log(`apex-forge browse — Fast headless browser for AI coding agents
  ...
  if (command === 'connect') {
    ...
    try {
      ...
      try {
        ...
        const agentQueue = path.join(process.env.HOME || '/tmp', '.apex', 'sidebar-agent-queue.jsonl');
