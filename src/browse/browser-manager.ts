...
export class BrowserManager {
  ...
  /**
   * Find the apex Chrome extension directory.
  ...
  private findExtensionPath(): string | null {
    ...
    const candidates = [
      ...
      // Global apex-forge install
      path.join(process.env.HOME || '', '.claude', 'skills', 'apex-forge', 'extension'),
      // Git repo root (detected via BROWSE_STATE_FILE location)
      (() => {
        const stateFile = process.env.BROWSE_STATE_FILE || '';
        if (stateFile) {
          const repoRoot = path.resolve(path.dirname(stateFile), '..');
          return path.join(repoRoot, '.claude', 'skills', 'apex-forge', 'extension');
        }
        return '';
      })(),
    ].filter(Boolean);
  ...
  async launch() {
    ...
    this.browser.on('disconnected', () => {
      ...
      console.error('[browse] Console/network logs flushed to .apex/browse-*.log');
      process.exit(1);
    });
  ...
  // ─── Headed Mode ─────────────────────────────────────────────
  /**
   * Launch Playwright's bundled Chromium in headed mode with the apex-forge
  ...
  async launchHeaded(authToken?: string): Promise<void> {
    ...
    // Find the apex-forge extension directory for auto-loading
    ...
    const fs = require('fs');
    const path = require('path');
    const userDataDir = path.join(process.env.HOME || '/tmp', '.apex', 'chromium-profile');
    ...
    const indicatorScript = () => {
      const injectIndicator = () => {
        if (document.getElementById('apex-ctrl')) return;

        const topLine = document.createElement('div');
        topLine.id = 'apex-ctrl';
        ...
        style.textContent = `
          ...
          @media (prefers-reduced-motion: reduce) {
            #apex-ctrl { animation: none !important; }
          }
        `;
