---
name: apex-forge-health
description: Code quality dashboard — wraps project tools, computes weighted composite 0-10 score, tracks trends over time
user-invocable: true
---

```bash
#!/usr/bin/env bash
# Apex Forge — Health Role Preamble
source "${APEX_ROOT:-$(cd "$(dirname "$0")/../.." && pwd)}/hooks/state-helper"

echo "=== APEX HEALTH ROLE ==="
apex_set_stage "health"

# ---------------------------------------------------------------------------
# Telemetry
# ---------------------------------------------------------------------------
apex_telemetry_start "health"
...
if command -v git &>/dev/null && git rev-parse --is-inside-work-tree &>/dev/null 2>&1; then
  ...
  echo "[health] Branch: $BRANCH"
else
  BRANCH="unknown"
  echo "[health] WARNING: Not a git repo"
fi
...
HEALTH_HISTORY=".apex/health-history.jsonl"

if [ -f "$HEALTH_HISTORY" ]; then
  HISTORY_COUNT=$(wc -l < "$HEALTH_HISTORY" 2>/dev/null | tr -d ' ')
  echo "[health] History entries: $HISTORY_COUNT"
else
  echo "[health] No history yet (first run)"
fi
...
> apex-forge / workflow / roles / health
>
...
> 0-10 score, and tracks trends over time.
...
Use Glob to search for shell scripts (`**/*.sh`) to confirm shell lint targets exist.

After auto-detection, present the detected tools to the user:
...
Append one JSONL line to `.apex/health-history.jsonl`:
...
## STEP 6: TREND ANALYSIS AND RECOMMENDATIONS

### Read History

```bash
tail -10 .apex/health-history.jsonl 2>/dev/null || echo "NO_HISTORY"
```

### Show Trend (if prior entries exist)
...
On first run (no history): "First health check -- no trend data yet. Run /health
again after making changes to track progress."
...
6. **Trends require history.** On first run, say so. No fabricated baselines.
...
| Status | When |
|--------|------|
| **DONE** | Dashboard produced with scores, history persisted |
| **DONE_WITH_CONCERNS** | Dashboard produced but some tools could not run (skipped categories) |
| **BLOCKED** | No tools detected and no Health Stack configured |
| **NEEDS_CONTEXT** | Cannot determine which tools to run; need user to specify |
