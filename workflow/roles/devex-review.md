---
name: apex-forge-devex-review
description: |
  Live developer experience audit. Actually TESTS the developer experience: navigates
  docs, tries the getting started flow, measures TTHW, screenshots error messages,
  evaluates CLI help text. Produces a DX scorecard with evidence. Compares against
  plan-devex-review scores if they exist (the boomerang: plan said 3 minutes,
  reality says 8). Use when asked to "test the DX", "DX audit", "developer experience
  test", or "try the onboarding".
user-invocable: true
---

# DX Live Audit

> apex-forge / workflow / roles / devex-review
>
...
>
> Use browser automation or manual verification to navigate docs, try the getting
> started flow, and screenshot what developers actually see. Use bash to try CLI
> commands. Measure, don't guess.
...
Browser-based testing can verify: docs pages, API playgrounds, web dashboards,
signup flows, interactive tutorials, error pages.

Browser-based testing CANNOT verify: CLI install friction, terminal output quality,
local environment setup, email verification flows, auth requiring real credentials,
offline behavior, build times, IDE integration.
...
1. Read CLAUDE.md / project docs for product URL, docs URL, CLI install command
...
Check for prior plan-devex-review scores. If prior scores exist, display them. These
are your baseline for the boomerang comparison at the end.

---

## Step 1: Getting Started Audit

Navigate to the docs/landing page. Screenshot it (via AF's browser-qa-testing role
...
- API playground: Navigate via browser if one exists. Screenshot.
...
Check (via browser or bash as appropriate):
...
|------------------|-----------|-----------|-------|---------|
| Getting Started  | __/10     | __/10     | __    | WARN/OK |
| API/CLI/SDK      | __/10     | __/10     | __    | WARN/OK |
| Error Messages   | __/10     | __/10     | __    | WARN/OK |
| Documentation    | __/10     | __/10     | __    | WARN/OK |
| Upgrade Path     | __/10     | __/10     | __    | WARN/OK |
| Dev Environment  | __/10     | __/10     | __    | WARN/OK |
| Community        | __/10     | __/10     | __    | WARN/OK |
| DX Measurement   | __/10     | __/10     | __    | WARN/OK |
| TTHW             | __ min    | __ min    | __ min| WARN/OK |
```
...
| Status | When |
|--------|------|
| **DONE** | All 8 audit steps completed, scorecard produced. |
| **DONE_WITH_CONCERNS** | Audit completed but some dimensions could not be tested. |
| **BLOCKED** | Cannot proceed (no URL, no access, product not running). |
| **NEEDS_CONTEXT** | Waiting for user to provide product URL or credentials. |
