---
name: apex-forge-plan-devex-review
description: |
...
> apex-forge / workflow / roles / plan-devex-review
>
...
- CLAUDE.md / project docs for project conventions
...
other review roles instead." Exit gracefully.
...
output section ("Developer Perspective") in the plan file.

### 0C. Competitive DX Benchmarking

Before scoring anything, understand how comparable tools handle DX. Use web search to
...
If web search is unavailable: "Search unavailable. Using reference benchmarks: Stripe
...
3. **Ask per friction point.** One question per friction point found. Do NOT batch
   multiple friction points into one question.
...
## Review Passes (8 passes, after Step 0 is complete)

**Anti-skip rule:** Never condense, abbreviate, or skip any review pass (1-8) regardless
of plan type. Every pass exists for a reason. If a pass genuinely has zero findings,
...
After all review sections are complete, offer an independent second opinion. Use
AF's codex-consult role for this (or a different AI system if available).

Ask the user:

> "All review sections are complete. Want an outside voice? A different reviewer can
> give an independent challenge of this plan -- logical gaps, feasibility risks, and
> blind spots. Takes about 2 minutes."
>
> A) Get the outside voice (recommended)
> B) Skip -- proceed to outputs

If A: Construct a review prompt with the plan content and persona context, then
invoke AF's codex-consult or an equivalent independent review mechanism.

Present findings. For any disagreements between the outside voice and the review,
flag as cross-model tension and let the user decide:
...
**User sovereignty:** Do NOT auto-incorporate outside voice recommendations. Present
each tension point. The user decides.
...
|----------------------|--------|--------|--------|
| Getting Started      | __/10  | __/10  | __ up/down |
| API/CLI/SDK          | __/10  | __/10  | __ up/down |
| Error Messages       | __/10  | __/10  | __ up/down |
| Documentation        | __/10  | __/10  | __ up/down |
| Upgrade Path         | __/10  | __/10  | __ up/down |
| Dev Environment      | __/10  | __/10  | __ up/down |
| Community            | __/10  | __/10  | __ up/down |
| DX Measurement       | __/10  | __/10  | __ up/down |
+--------------------------------------------------------------------+
| TTHW                 | __ min | __ min | __ up/down |
...
| Overall DX           | __/10  | __/10  | __ up/down |
+====================================================================+
...
Additional rules for DX reviews on top of normal AF question format:
...
- **Escape hatch:** If a section has no issues, say so and move on. If a gap has an
  obvious fix, state what you'll add and move on.
...
- **Engineering review** if DX issues have architectural implications
- **Design review** if user-facing UI exists
- **devex-review (live audit) after implementation** -- the boomerang. Plan said TTHW
  would be [target from 0C]. Did reality match?
...
| Status | When |
|--------|------|
| **DONE** | All 8 passes completed, scorecard and outputs produced. |
| **DONE_WITH_CONCERNS** | Review completed but some passes had unresolved decisions. |
| **BLOCKED** | Cannot proceed (no plan file, product type not developer-facing). |
| **NEEDS_CONTEXT** | Waiting for user to provide plan file or persona info. |
