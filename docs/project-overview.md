...
```
Skills 层（主体）— 60 个 markdown 文件
  AI Agent 读了照着做。协议规则、pipeline 阶段、领域能力、操作命令都在这里。
  这是 AF 的核心工作方式。

CLI 层 — apex 命令行工具
  处理状态持久化：任务队列、记忆存储、遥测统计、可视化 dashboard。
  Agent 通过 shell 调用 apex 命令读写 .apex/ 目录下的数据。

MCP 层（可选）— apex-forge-mcp
  把 CLI 的功能暴露为 MCP 工具，IDE 里可以直接调用。
  不配也完全能用，Agent 通过 shell 调 CLI 达到同样效果。
```
